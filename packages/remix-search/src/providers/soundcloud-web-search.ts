...
export const soundCloudWebSearchProvider: RemixSearchProviderModule = {
  ...
  async search(context: RemixSearchContext) {
    let searxngSearchUrl = getSearxngSearchUrl();

    logRemixProvider("soundcloud-web", "search started", {
    ...
    for (const query of context.queries.slice(0, 8)) {
      ...
      logRemixProvider("soundcloud-web", "query started", {
      ...
      logRemixProvider("soundcloud-web", "query completed", {
        query: soundCloudQuery,
      ...
      for (const result of results) {
        ...
        candidates.push(
          await mapSearchResultToCandidate(soundCloudUrl, result),
        );
      }
    }

    logRemixProvider("soundcloud-web", "search completed", {
...
async function mapSearchResultToCandidate(
  ...
  return {
    title,
    artists,
    remixArtist: extractRemixArtist(title) ?? artists,
    album: null,
