...
- **Tool**: Capability directly callable by an LLM or agent orchestrator, such as a
  LangChain tool, OpenAI function-calling tool, MCP tool, or Agents SDK tool.
- **Agent**: Autonomous LLM-driven conversational flow with iterative reasoning and tool-calling.
...
- Use `Tool` only for capabilities exposed directly to an LLM or agent orchestrator.
- Use `Agent` only for autonomous LLM flows that can iteratively call tools.
